...
- [ ] the rest of the route table: 266 of 487 bound by hand, 228 drawn by the
...
- [x] the screens' own words in the dictionary — all of them. No `title=`,
      `label=`, `description=` or `subtitle=` literal is left under
      `src/features/` or `src/components/`, the compiler keeps the two
      dictionaries in step, and a third locale is a file of the same shape.
      A different thing from what the shop *sells* being translated, which is
      also done: a product's and a category's own words in any locale
