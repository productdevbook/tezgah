...
pub async fn due(
    ...
    Ok(Page::build(rows, paging, |row| {
        Cursor::at(row.next_billing_at, row.id.as_uuid())
    }))
}
...
pub async fn events(
    ...
    Ok(Page::build(rows, paging, |row| {
        Cursor::at(row.at, row.id.as_uuid())
    }))
}
...
pub async fn due_deliveries(
    ...
    Ok(Page::build(rows, paging, |row| {
        Cursor::at(
            row.next_delivery_at.unwrap_or(row.created_at),
            row.id.as_uuid(),
        )
    }))
}
...
impl Step for ReserveStock {
    ...
    async fn compensate(&self, tx: &mut Tx<'_>, ctx: &Ctx<'_>, keep: &Value) -> Result<()> {
        ...
        for id in undo.reservations {
            // `create_order` binds this same reservation onto an order line
            // and its own compensation gives it back first when it runs, so
            // by the time this one unwinds the row may already be gone
            // (#148's tolerance, same reason checkout's compensate needs it).
