...
/// needs to fill a shop, a region's own edit, and the reads past what the
/// panel has a screen for yet (`admin::router`'s own doc comment names all
/// five) — requires that same bearer token: see `admin::router`'s doc
