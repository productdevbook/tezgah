...
/// needs to fill a shop, a product's own edit and delete, and the reads past
/// what the panel has a screen for yet (`admin::router`'s own doc comment
/// names all five) — requires that same bearer token: see `admin::router`'s
/// doc comment for why an unset token means no admin surface at all rather
/// than an open one.
